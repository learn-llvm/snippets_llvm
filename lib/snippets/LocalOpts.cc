...
template <>
bool valEquals(ConstantFP *L, APFloat *R) {
  return L->getValueAPF().compare(*R) == APFloat::cmpEqual;
}
...
struct LocalOpts : public FunctionPass {
  ...
  // Replace all uses of an instruction with the value val. Also delete
  // instruction.
  // At the end, the iterator i points to the instruction just before the one
  // that was removed.
  ...
  // For constant folding integer operators. Simply perform the relevant
  // operation on the operands.
  ConstantInt *evalBinaryIntOp(unsigned op, ConstantInt *left,
                               ConstantInt *right) {
    switch (op) {
  ...
  // For constant folding float operators. Simply perform the relevant operation
  // on the operands.
  // This will silently ignore errors like overflow/underflow
  ConstantFP *evalBinaryFloatOp(unsigned op, ConstantFP *left,
                                ConstantFP *right) {
  ...
  bool multiplyToShift(BasicBlock::iterator &i, ConstantInt *Op1, Value *Op2) {
    ...
    if (multiple.isPowerOf2()) {
      ...
      BinaryOperator *newInst = BinaryOperator::Create(
          Instruction::Shl, Op2, ConstantInt::get(Op1->getType(), lg, false));
    ...
    for (BasicBlock::iterator i = bb.begin(), e = bb.end(); i != e; ++i) {
      ...
      switch (op) {
        ...
        case Instruction::Store:
          ...
          // If a constant is being stored into a variable, find the place where
          // it is being
          // loaded into a register, and replace all uses of the register with
          // the constant.
          {
            Value *ptr = cast<StoreInst>(i)->getPointerOperand();
            Value *val = cast<StoreInst>(i)->getValueOperand();
            if (isa<Constant>(*val)) {
              for (Value::use_iterator u = ptr->use_begin();
                   u != ptr->use_end(); ++u) {
        ...
        case Instruction::Add: {
          if (applyIdentity(i, commIdentities<ConstantInt, APInt>(
                                   L, R, &zeroAPI, nullptr))) {
            // a + 0 = 0 + a = a
            optinf.algebraic++;
            modified = true;
        ...
        case Instruction::FAdd: {
          if (applyIdentity(i, commIdentities<ConstantFP, APFloat>(
                                   L, R, &zeroAPF, nullptr))) {
            // a + 0 = 0 + a = a
            optinf.algebraic++;
            modified = true;
        ...
        case Instruction::Sub:
          // Algebraic identities
          {
            ...
            if (applyIdentity(i, selfInverse<ConstantInt>(L, R, zeroCI))) {
              ...
              modified = true;
              continue;
            } else if (applyIdentity(i, constIdentity<ConstantInt, APInt>(
                                            R, L, &zeroAPI, nullptr))) {
              // a - 0 = a
              optinf.algebraic++;
              modified = true;
        ...
        case Instruction::FSub: {
          ...
          if (applyIdentity(i, selfInverse<ConstantFP>(L, R, zeroCF))) {
            ...
            modified = true;
            continue;
          } else if (applyIdentity(i, constIdentity<ConstantFP, APFloat>(
                                          R, L, &zeroAPF, nullptr))) {
            // a - 0 = a
            optinf.algebraic++;
            modified = true;
        ...
        case Instruction::Mul: {
          if (applyIdentity(i, commIdentities<ConstantInt, APInt>(L, R, &oneAPI,
                                                                  &zeroAPI))) {
            // a * 1 = 1 * a = a
            // a * 0 = 0 * a = 0
            optinf.algebraic++;
        ...
        case Instruction::SRem: {
          ...
          if (applyIdentity(i, selfInverse(L, R, zeroCI))) {
            ...
            modified = true;
            continue;
          } else if (applyIdentity(i, remIdentity<ConstantInt, APInt>(
                                          L, R, &oneAPI, &zeroAPI))) {
            // a mod 1 = 0
            optinf.algebraic++;
            modified = true;
        ...
        case Instruction::FRem: {
          ...
          if (applyIdentity(i, selfInverse(L, R, zeroCF))) {
            ...
            modified = true;
            continue;
          } else if (applyIdentity(i, remIdentity<ConstantFP, APFloat>(
                                          L, R, &oneAPF, &zeroAPF))) {
            // a mod 1 = 0
            optinf.algebraic++;
            modified = true;
  ...
  virtual bool runOnFunction(Function &f) override {
    ...
    for (auto &elem : f) {
      modified = runOnBasicBlock(elem, optinf);
    }
