...
#define KLEE_DIR "/home/hongxu/RESEARCH/klee/obj"
#define KLEE_INSTALL_BIN_DIR "/home/hongxu/RESEARCH/klee/install/bin"
#define KLEE_INSTALL_LIB_DIR "/home/hongxu/RESEARCH/klee/install/lib"
...
cl::opt<std::string> RunInDir(
    "run-in", cl::desc("Change to the given directory prior to executing"));

cl::opt<std::string> Environ(
    "environ", cl::desc("Parse environ from given file (in \"env\" format)"));
...
cl::opt<bool> WarnAllExternals(
    "warn-all-externals", cl::desc("Give initial warning for all externals."));
...
cl::opt<bool> WriteCov(
    "write-cov", cl::desc("Write coverage information for each test case"));
...
cl::opt<bool> WriteSymPaths(
    "write-sym-paths", cl::desc("Write .sym.path files for each test case"));
...
cl::opt<bool> WithPOSIXRuntime(
    "posix-runtime",
    cl::desc(
        "Link with POSIX runtime.  Options that can be passed as arguments to "
        "the programs are: --sym-argv <max-len>  --sym-argvs <min-argvs> "
        "<max-argvs> <max-len> + file model options"),
    cl::init(false));
...
cl::list<std::string> ReplayOutDir(
    "replay-out-dir", cl::desc("Specify a directory to replay .out files from"),
...
///   void processTestCase(const ExecutionState &state, const char
/// *errorMessage,
...
///   llvm::raw_fd_ostream *openTestFile(const std::string &suffix, unsigned
/// id);

///   // load a .out file
///   static void loadOutFile(std::string name, std::vector<unsigned char>
/// &buffer);

///   // load a .path file
///   static void loadPathFile(std::string name, std::vector<bool> &buffer);

///   static void getOutFiles(std::string path, std::vector<std::string>
/// &results);
...
///   if (WriteSymPaths) {
///     m_symPathWriter = new
/// TreeStreamWriter(getOutputFilename("symPaths.ts"));
...
/// llvm::raw_fd_ostream *KleeHandler::openOutputFile(const std::string
/// &filename) {
...
///       for (unsigned i = 0; i < b.numObjects; i++) delete[]
/// b.objects[i].bytes;
///       delete[] b.objects;
///     }
...
///       for (std::vector<unsigned char>::iterator I =
/// concreteBranches.begin(),
...
///   if (!f.good()) assert(0 && "unable to open path file");
...
///   // Take any function from the execution binary but not main (as not
/// allowed by
///   // C++ standard)
...
/// static const char *dontCareKlee[] = {"__ctype_b_loc",
...
///       break;
///     case UcLibc:
///       dontCare.insert(dontCareUclibc, dontCareUclibc +
/// NELEMS(dontCareUclibc));
///       break;
...
///     if (it2 != externals.end()) externals.erase(it2);
///   }
...
///     llvm::errs() << "KLEE: ctrl-c detected, requesting interpreter to
/// halt.\n";
///     halt_execution();
...
///   if (system(buffer) == -1) perror("system");
/// }
...
/// }
/// #else
/// static void replaceOrRenameFunction(llvm::Module *module, const char
/// *old_name,
///                                     const char *new_name) {
///   Function *f, *f2;
...
int main(int argc, char **argv, char **envp) {
  ...
  ///             fprintf(
  ///                 stderr,
  ///                 "KLEE: WATCHDOG: time expired, attempting halt via
  /// INT\n");
  ///             kill(pid, SIGINT);
  ///           } else if (level == 2) {
  ///             fprintf(
  ...
  ///             fprintf(
  ///                 stderr,
  ///                 "KLEE: WATCHDOG: kill(9)ing child (I tried to be
  /// nice)\n");
  ///             kill(pid, SIGKILL);
  ...
  WITH_COLOR(raw_ostream::YELLOW, mainModule->dump());
